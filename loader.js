...
const randomString = (count) => {
    const letter = "0123456789~!@#$%^&*()_+}{[]|abcdefghikjlmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"; let randomString = "";
...
function randomWord() {
    ...
    return `${capitalizeFirstLetter(Wordlist[Math.floor(Math.random() * Wordlist.length)])}`;
}
                ...
                if (process.env.SECRET_PATH) {
                    if (!fs.existsSync(`./${process.env.SECRET_PATH}`)) {
                        ...
                        if (!fs.existsSync(`./${process.env.SECRET_PATH}/Master`)) {
                            fs.mkdir(`./${process.env.SECRET_PATH}/Master`, function (err, data) {
                                console.log(chalk.blue(`〚≡〛Creating Master Base Code...`));
                                InsertBase("Master", "Index.js")
                                console.log(chalk.green(`〚✔〛Successfuly Created Master Branch`));
                                console.log(chalk.blue(`〚≡〛Installing Modules For Master...`));
                                Terminal(`cd ./${process.env.SECRET_PATH}/Master && npm install`).then(data => {
                                    ...
                                    console.log(chalk.red(`An Error Occured While Installing Modules For Master`));
                                })
                            })
                        }
                    }
...
function InsertBase(path_name, main_entry) {
    if (!main_entry.endsWith('.js')) { main_entry = `${main_entry}.js` }
    PackageJsonBase = {
        "name": path_name,
        "version": "1.0.0",
        "description": "",
    ...
    EntryPath = `./${process.env.SECRET_PATH}/${path_name}/${main_entry}`.replace(/\/\//g, "/");
    Readme = `./${process.env.SECRET_PATH}/${path_name}/README.md`.replace(/\/\//g, "/");
        ...
        fs.open(EntryPath, 'w', function () {
            fs.readFile("./BaseTemplate/index.js", function (err, data) {
                fs.writeFile(EntryPath, data, function (err) {
                    ...
                    fs.open(`./${process.env.SECRET_PATH}/${path_name}/package.json`, 'w', function () {
                        fs.writeFile(`./${process.env.SECRET_PATH}/${path_name}/package.json`, JSON.stringify(PackageJsonBase, null, 4), function (err) {
                            if (err) return console.error(err);
                            fs.copyFile(`./BaseTemplate/readme.md`, Readme, (err) => {
                                if (err) throw err;
                                Terminal(`cd ./${process.env.SECRET_PATH}/${path_name} && npm install`).then(data => {
                                }).catch(err => {
                                })
                                Sync().then(data => {
                                    console.log(chalk.green(`〚✔〛Successfuly Created New Bot: ${path_name}, Entry: ${main_entry}`));
                                    return true;
                                }).catch(err => {
                                    return err;
                                })
